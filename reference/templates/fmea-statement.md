...
| **Occurrence** | Evidence the assumption *is* wrong: observed, boundary-adjacent, or conceivable-only. Observed claims contain independently checkable citations (incident record, `⊨` entry, failing run, measurement log) or they downgrade. |
| **Detection** | Whether being wrong would surface on its own, and what the fix costs once it does. |
| **Cost** | The **bound** placed on the act. For a launch: which files it may read or edit, which commands or probes it may run, token cap(s), plus the stopping condition ("past N files, stop and report"). For a bound amendment: the bound's full replacement text, in the form `governing-work` authors and the governor consumes — decidable by a reader possessing only the produced content and the bound, measured, never a delta. |
