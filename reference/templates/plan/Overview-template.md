...
<Shape. An enumerated list, one limit per item, each stated so that a reader holding only the produced work can decide whether it was crossed: a countable threshold, a named file or directory set, a construct that must not appear, an artifact that must exist. Where the limit has a source — the PRD, the Design, a directive or rule, a decision recorded during planning — quote the source's own words and cite where they came from. A paraphrase drifts from what was actually agreed, and the drift is invisible by the time anyone checks.>
