...
**※6. Memory Curation** — Read memory progressively, loading only material relevant to the current task. Route memory writes, consolidation, and durable-knowledge promotion to the knowledge agent; the main session identifies what to retain but does not curate the memory subsystem.
