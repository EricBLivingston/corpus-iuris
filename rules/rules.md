...
**※8. Production Workflow Chain** — Non-trivial work runs analyzer → coder → reviewer → tester, sized under ⊨5. Testing includes the §16 cross-boundary end-to-end gate wherever applicable. The review cycle is a remedial cycle under ⊨7. Meta-artifacts created by the workflow are exempt from this chain (no recursion). Independent review, non-code refactors included, is never skippable.
