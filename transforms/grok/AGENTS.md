...
A skill takes Grok's discovery admission instead of a rules copy. A command stays a command: the file lands under `{command-root}/` as flat markdown. On that copy, drop any Claude `model` frontmatter slug (`fable`, `opus`, `sonnet`); keep `argument-hint` and `description`. That drop is a copy-time correction, not a second generator. An agent takes Grok's agent-discovery channel — the transposed markdown in `src/agents/` — never a rules copy.
...
4. Copy any `commands/<name>.md` into staging under `commands/`, dropping the Claude `model` frontmatter slug if present. Inspect each command before selecting it.
...
8. Promote only validated staged outputs to their live destinations: selected rules to `~/.grok/rules/`, reference material to `~/.grok/corpus/reference/`, each skill directory to `~/.grok/skills/<name>/`, each command to `~/.grok/commands/`, and transposed agents to `~/.grok/agents/`. Do not copy the whole checkout and do not delete unselected resident files. Do not edit the adopter's `AGENTS.md`. Update the non-resident adoption ledger with source identity and retained corrections.
