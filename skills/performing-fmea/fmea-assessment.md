...
Fill the ATO row of the statement file at the absolute path your dispatch prompt gave you, in place. The row is already there and empty: grant or denial, with reasoning either way, at the length the template asks for. Author nothing else — no second document, no report file, no restated table. A grant authorizes the act at the Cost bound restated in the ATO row and nothing wider; crossing it requires a fresh statement. A denial names the row that failed, binds — the requestor narrows and resubmits, or drops — and closes with one of two outputs: the text you would grant in that row's place, which a resubmission adopts verbatim, or a declaration that no statement on this act would be granted, and why, which ends the matter.

A denial is recorded exactly as a grant is. Never leave the row empty or the file deleted because nothing proceeded; the denials are what the record exists to measure.

Return the same row content, and nothing else, in your final message: the dispatching session is blocked on it.
